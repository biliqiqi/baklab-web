...
const fontSizeSchema = z.enum(fontSizeList)
...
const contentWidthSchema = z.enum(contentWidthList)
...
const UserUIForm = () => {
  ...
  useEffect(() => {
    if (formVals.contentWidth == 'custom') {
      setUserUIState({
        contentWidth:
          Number(formVals.customContentWidth) || Number(DEFAULT_CONTENT_WIDTH),
      })
    } else {
      setUserUIState({
  ...
  return (
    <Form {...form}>
      {/* @ts-expect-error zod enum problem */}
